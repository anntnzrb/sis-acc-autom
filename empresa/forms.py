...
class EmpresaForm(forms.ModelForm):
    ...
    class Meta:
        ...
        widgets = {
            ...
            'anio_fundacion': forms.NumberInput(attrs={
                'class': 'form-control',
    ...
    def __init__(self, *args, **kwargs):
        ...
        for field_name in ['nombre', 'direccion', 'mision', 'vision', 'anio_fundacion', 'ruc']:
    ...
    def clean_mision(self):
        ...
        if mision:
            mision = mision.strip()
            if len(mision) < 20:
                raise ValidationError("La misión debe ser más descriptiva (al menos 20 caracteres).")
        return mision
    ...
    def clean_vision(self):
        ...
        if vision:
            vision = vision.strip()
            if len(vision) < 20:
                raise ValidationError("La visión debe ser más descriptiva (al menos 20 caracteres).")
        return vision
    ...
    def clean_anio_fundacion(self):
        ...
        if anio:
            ...
            if anio < 1900:
                raise ValidationError("El año de fundación no puede ser anterior a 1900.")
            if anio > current_year:
                raise ValidationError(f"El año de fundación no puede ser mayor al año actual ({current_year}).")
        return anio
    ...
    def clean_ruc(self):
        ...
        if ruc:
            ...
            if self.instance.pk:
                ...
                if Empresa.objects.filter(ruc=ruc).exclude(pk=self.instance.pk).exists():
