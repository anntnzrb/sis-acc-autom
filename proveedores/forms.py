    ...
    def clean_descripcion(self):
        ...
        if descripcion:
            ...
            if len(descripcion) < 10:
                raise ValidationError(
                    "La descripción debe ser más detallada (al menos 10 caracteres)."
                )
        return descripcion
    ...
    def clean_pais(self):
        ...
        if pais:
            ...
            if not pais.replace(" ", "").replace("-", "").isalpha():
                raise ValidationError(
                    "El país solo puede contener letras, espacios y guiones."
                )
        return pais
    ...
    def clean(self):
        ...
        if nombre:
            if self.instance.pk:
                ...
                existing = Proveedor.objects.filter(nombre__iexact=nombre).exclude(
                    pk=self.instance.pk
                )
                if existing.exists():
                    raise ValidationError(
